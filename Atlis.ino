...
//bytes 24 - 31 LimiterAdress
//bytes 32 - 35 float limiterMax
...
//Adress of the temperature limiting senros thormometer
...
void setup()
{
  ...
  swCmd.addCommand("T",Tcmd); // set Target Temp
  ...
  swCmd.addCommand("L",Lcmd); // set limiter thermometer
  swCmd.addCommand("M",Mcmd); // set limiter max
  ...
  EEPROM.get(24, LimiterAddress);
  EEPROM.get(32, limiterMax);
  
  SWprint("Control Address from EEPROM: ");
  printAddress(ControlAddress);
  SWprintln("");

  SWprint("Limiter Address from EEPROM: ");
  ...
  if (LimiterDevice>-1) {
    SWprint("Limiter themometer is: ");
    SWprinti(LimiterDevice);
    SWprintln("");
  } else {
    SWprintln("No limiter thermometer! Set with L.");
  }
...
void Lcmd() {
  ...
  arg = swCmd.next();
  if (arg != NULL) {
    LimiterDevice=atoi(arg);
    if (LimiterDevice>numDev-1 || LimiterDevice < 0) {
      LimiterDevice=-1;
      SWprintln("L: Device does not exist.");
    } else {
      for (int i=0;i<8;i++) {
        LimiterAddress[i] = Thermometer[LimiterDevice][i];
      }
      EEPROM.put(24,Thermometer[LimiterDevice]);
    }
  } else {
    SWprintln("L: No data.");
  }
...
void Mcmd() {
  ...
  if (arg != NULL) {
    limiterMax=atof(arg);
    SWprint("Limiter temperature set to: ");
    SWprintd(limiterMax);
    SWprintln("");
    EEPROM.put(32,limiterMax);
  } else {
    SWprintln("M: No data.");
  }
}
